...
class SingularityIndexFilters:
    ...
    def _createFilters(self):
        ...
        X, Y = np.meshgrid(
            list(range(-ksize2, ksize2 + 1)), list(range(-ksize2, ksize2 + 1))
        )
...
def process_scale(
    ...
    if s > 0:
        I1 = cv2.resize(
            I1,
            (int(C / (np.sqrt(2) ** s)), int(R / (np.sqrt(2) ** s))),
    ...
    J0 = cv2.sepFilter2D(
        I, cv2.CV_64FC1, filters.G01d, filters.G01d.T, borderType=cv2.BORDER_REFLECT_101
    )
    J1 = J0u * np.cos(angles) + J90u * np.sin(angles)
    J2 = (
        (1 + (2 * np.cos(2 * angles))) * J20
        + (1 - np.cos(2 * angles) + (np.sqrt(3) * np.sin(2 * angles))) * J260
        + (1 - np.cos(2 * angles) - (np.sqrt(3) * np.sin(2 * angles))) * J2120
    ) / 3
...
def applyMMSI(I1, filters, togglePolarity=False, narrow_rivers=True):
    ...
    A = (
        s_next * (psi_max - psi_max_prev)
        + s_max * (psi_max_prev - psi_max_next)
        + s_prev * (psi_max_next - psi_max)
    )
    B = (
        s_next * s_next * (psi_max_prev - psi_max)
